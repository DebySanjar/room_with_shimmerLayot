...
class BoyFragment() : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        _binding?.profileImage!!.setImageResource(R.drawable.img_1)

        // Gender bo'yicha count olish
        personDao.getCountGender("Erkak").observe(viewLifecycleOwner) { count ->
            binding.username.text = "Erkaklar : ${count.toString()}"
        }

        // LiveData orqali kuzatamiz
        personDao.getPersonsByGender("Erkak").observe(viewLifecycleOwner) { list ->
